 ...
 */
const eddyproDto : (startDatetime: string, endDatetime: string, type: string) => Promise<any> = async (startDatetime, endDatetime, type) => {
    ...
    let query = new String(
        `
            from(bucket: "${config.bucket}")
              ...
              |> filter(fn: (r) => r._measurement == "eddypro")
